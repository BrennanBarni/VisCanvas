	...
	*/
	...
	for (int i = 0; i < getDimensionAmount(); i++) {
		if (isDimensionInverted(i)) {
			saveFile << "Invert Dimension," << isDimensionInverted(i) << "\n";
		}
	}
...
void DataInterface::parseLine(std::vector<std::string>* lineTokens) {
	...
	// read data
		...
		if (lineTokens->size() >= 2) {
			int dimensionIndex = stod((*lineTokens)[1]);
			invertDimension(dimensionIndex);
		}
	}
	else if ((*lineTokens)[0].compare("Read Set Names") == 0) {
		if (lineTokens->size() >= 2) {
			bool isReadSetNames = stod((*lineTokens)[1]);
			setReadSetNames(isReadSetNames);
		}
	}
	else if ((*lineTokens)[0].compare("Read Dimension Names") == 0) {
		if (lineTokens->size() >= 2) {
			bool isReadDimensionNames = stod((*lineTokens)[1]);
			setReadDimensionNames(isReadSetNames);
		}
	}
	else if ((*lineTokens)[0].compare("Read Class Names") == 0) {
		if (lineTokens->size() >= 2) {
			bool isReadClassNames = stod((*lineTokens)[1]);
			setReadClassNames(isReadSetNames);
		}
	}
	/*
